...
function buildCalc(income, ded, _skipDirectorCalc) {
  ...
  const totalGrossIncome = sSalary + sCommission + sRoyalty + s4Interest + s4Dividend + rAmt + pAmt + bAmt + directorBenefitTaxable + directorGrossUpForBase;
  const nonSalaryIncome240to8 = sCommission + sRoyalty + s4Interest + s4Dividend + rAmt + pAmt + bAmt;
  ...
  const withholdingPaid = num(income.whtSalary) + num(income.whtRoyalty) + num(income.whtRental) + num(income.whtProf) + num(income.whtBiz);
      ...
      <div style={{ maxWidth: 880, margin: "0 auto", padding: "30px 20px 90px" }}>
        ...
        <div>
          ...
          <StaticCard en="กลุ่ม 6" th="อื่นๆ และมาตรการกระตุ้นเศรษฐกิจ">
            ...
            <div style={{ marginTop: 14, fontSize: 16, fontWeight: 700, color: "#1B5E3A", background: "#E4F5E6", border: "2px solid #2CA8D8", borderRadius: 5, padding: "12px 14px", lineHeight: 1.7 }}>
              ⚠ มาตรการกระตุ้นเศรษฐกิจสำหรับปีภาษี 2569 ขึ้นอยู่กับประกาศของภาครัฐซึ่งยังไม่ยืนยันรายละเอียด/วงเงินสุดท้าย กรอกเฉพาะกรณีทราบวงเงินที่ประกาศแล้วเท่านั้น
            </div>
          </StaticCard>
          )}
